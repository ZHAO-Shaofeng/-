...
* [CSS常用](#CSS常用)
	...
	4.[渐变](#4渐变)
	5.[flex页脚置底(兼容至ie10)](#5flex页脚置底(兼容至ie10))
	6.[css三角形](#6css三角形)
	7.[tab常用响应式不转行，可左右拉动](#7tab常用响应式不转行，可左右拉动)
	8.[让背景图随DIV变化，且不变形居中](#8让背景图随DIV变化，且不变形居中)
* [JS常用](#JS常用)
    	1.[获取URL参数值](#1获取URL参数值)
	2.[头部提取之后，导航跳转添加对应的active ](#2头部提取之后，导航跳转添加对应的active)
